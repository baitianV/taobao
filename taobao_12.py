...
class tb_spider(object):
    ...
    def check_login(self):
        ...
        flag=bs.find_all('li',id='J_LoginInfo')
    ...
    def spider_core(self):
        ...
        try:
            ...
            for i in range(totalPage,0,-1):
                ...
                for shopItem in shopItems:
                    ...
                    if rank_res['rank']>0:
                        ...
                        elif rank_res['type']=='天猫':
                            tm_rank_list.append(rank_res)
                        rank_count+=1
                if rank_count==0:
                    key_count+=1
                else:
            ...
            file_name=self.key_word.replace(' ', '_')+'.txt'
            with open('./爬取结果/淘宝_'+file_name,'w') as f:
                for it in tb_rank_list:
                    f.write(str(it)+'\n')
            with open('./爬取结果/天猫_'+file_name,'w') as f:
                for it in tm_rank_list:
                    f.write(str(it)+'\n')        
...
def do_sleep():
    t=random.randint(5,10)*0.1+1.5
    time.sleep(t)

def long_sleep():
    t=random.randint(5,10)*0.3+10
    time.sleep(t)             
